defmodule Hazel.Torrent.Store do
  ...
  def init({peer_id, info_hash, opts}) do
    children = [
      worker(Store.BitField, [peer_id, info_hash, Keyword.take(opts, [:length, :piece_length])]),
      worker(Store.File, [peer_id, info_hash, Keyword.take(opts, [:name, :pieces, :length, :piece_length])]),
      # todo, better name?
      supervisor(Store.Processes, [peer_id, info_hash, Keyword.take(opts, [:length, :piece_length])])
    ]
