defmodule Hazel.Torrent.SwarmFaux do
  ...
  def start_link(peer_id, info_hash, opts \\ []) do
    opts = Keyword.put(opts, :pid, self)
    opts = Keyword.put(opts, :info_hash, info_hash)
    opts = Keyword.put(opts, :peer_id, peer_id)
  ...
  def handle_call(message, _from, state) do
    [command|args] = Tuple.to_list(message)
    if is_function(state[:cb][command]), do: apply(state[:cb][command], args ++ [state])
    {:reply, :ok, state}
  end
end
